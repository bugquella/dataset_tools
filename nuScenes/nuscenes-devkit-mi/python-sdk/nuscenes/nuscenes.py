...
class NuScenesExplorer:
    ...
    def map_pointcloud_to_image(self,
        ...
        # First step: transform the pointcloud to the ego vehicle frame for the timestamp of the sweep.将点云转换到ego坐标系
        print('-------')
        cs_record = self.nusc.get('calibrated_sensor', pointsensor['calibrated_sensor_token'])
        pc.rotate(Quaternion(cs_record['rotation']).rotation_matrix)
        pc.translate(np.array(cs_record['translation']))

        # Second step: transform from ego to the global frame. ego坐标系转全局坐标系
        ...
        # Fourth step: transform from ego into the camera.  ego坐标系转camera
        cs_record = self.nusc.get('calibrated_sensor', cam['calibrated_sensor_token'])
        pc.translate(-np.array(cs_record['translation']))
        pc.rotate(Quaternion(cs_record['rotation']).rotation_matrix.T)
        print(pointsensor['filename'], cam['filename'], cs_record['camera_intrinsic'], cam['calibrated_sensor_token'])
